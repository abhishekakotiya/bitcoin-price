...
def computeDelta(wt, X, Xi):
    ...
    # YOUR CODE GOES HERE
    x = X.values
    x, y = np.array(x[:-1]), x[-1]

    numerator = 0.0
    denominator = 0.0
    for row in Xi.itertuples():
    	xi, yi = np.array(row[1:-1]), row[-1]
    	sim_measure = (x - np.mean(x)) * (xi - np.mean(xi)) 
    	sim_measure = np.sum(sim_measure) / (len(x) * np.std(x) * np.std(xi))

    	numerator += (yi * np.exp(wt*sim_measure))
    	denominator += np.exp(wt*sim_measure)
